...
from app.schemas.quiz import BattleAnswerResponse, BattleAnswerSubmit, BattleCreate, BattleDetailResponse
...
async def create_battle(db: AsyncSession, tenant_id: uuid.UUID, current_user: User, body: BattleCreate):
    opponent_result = await db.execute(
        select(User).where(User.id == body.opponent_id, User.tenant_id == tenant_id)
    )
    opponent = opponent_result.scalar_one_or_none()
    if opponent is None:
        raise AppError(ErrorCode.OPPONENT_NOT_FOUND)

    if body.opponent_id == current_user.id:
        raise AppError(ErrorCode.CANNOT_CHALLENGE_SELF)
...
async def list_battles(db: AsyncSession, tenant_id: uuid.UUID, current_user: User):
    result = await db.execute(
        select(QuizBattle)
        .where(
            QuizBattle.tenant_id == tenant_id,
...
async def get_battle(db: AsyncSession, tenant_id: uuid.UUID, current_user: User, battle_id: uuid.UUID):
    battle = await db.get(QuizBattle, battle_id)
    if battle is None or battle.tenant_id != tenant_id:
        raise AppError(ErrorCode.BATTLE_NOT_FOUND)
...
async def submit_answer(db: AsyncSession, tenant_id: uuid.UUID, current_user: User, battle_id: uuid.UUID, body: BattleAnswerSubmit):
    battle = await db.get(QuizBattle, battle_id)
    if battle is None or battle.tenant_id != tenant_id:
        raise AppError(ErrorCode.BATTLE_NOT_FOUND)
    ...
    if await _maybe_expire_battle(battle):
        await db.commit()
        raise AppError(ErrorCode.BATTLE_EXPIRED)

    if battle.status == "completed":
        raise AppError(ErrorCode.BATTLE_COMPLETED)
    ...
    if existing_result.scalar_one_or_none() is not None:
        raise AppError(ErrorCode.ALREADY_ANSWERED)
        ...
        if challenger_answer and opponent_answer:
            ...
            if battle.winner_id:
                winner_result = await db.execute(
                    select(User).where(
                        User.id == battle.winner_id, User.tenant_id == tenant_id
                    )
                )
